...
import {addLoanType, clientLoanStatment, clientsLoans, deleteLoanType, returnLoan, updateLoan, updateLoanType} from '../api/loan/api.js'
import { useTokenContext } from '../../context/TokenContext.js';
import Spinner from './Spinner.js'
import Link from 'next/link.js';
        

export default function ClientStatment(props) {

    const {token} = useTokenContext()
    let info = {id:props.id,token}

    const {isLoading,data} = useQuery({
        queryKey:['loans-client-statement'+props.id],queryFn:async ()=> await clientLoanStatment(info)
      })
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                ...
                <Link href={`/preview-loan/${rowData.customer.id}`}>
     ...
     <div>
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={data?.results} 
                    ...
                    <Column field="debit" header="Debit" body={maxBodyTemplate}></Column>
                    <Column field="debit" header="Balance" body={balanceBodyTemplate}></Column>
